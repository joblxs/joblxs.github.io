...
$.ajax({
  ...
  data: {
    key: '2WJBZ-IMR6L-EEUP4-M74WF-LB7XT-EKF6I',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(116.315231, 39.978294, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Fomalhaut🥝 %c 你正在访问lxshuai🥝の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| lxshuai🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('lxshuai🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("04/13/2024 00:00:00");
  ...
  1 == String(snum).length && (snum = "0" + snum);
    ...
    (document.getElementById("workboard").innerHTML = `<div style="font-size:13px;font-weight:bold">本站已经运行 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`);
}
// 设置重复执行函数，周期1000ms
setInterval(() => {
  createtime();
